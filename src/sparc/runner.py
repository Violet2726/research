...
def run_experiment(
    ...
    metrics_payload = _build_metrics(all_prediction_rows, experiment.experiment_kind)
    diagnostics_payload = _build_diagnostics(all_prediction_rows, trigger_selection, experiment.experiment_kind)
...
def _run_sample(
    ...
    if experiment.experiment_kind == "content_ablation":
        return _run_content_sample(shared_context)
    if experiment.experiment_kind == "auditing_ablation":
    ...
    raise ValueError(f"Unsupported experiment_kind: {experiment.experiment_kind}")
...
def _build_metrics(prediction_rows: list[dict[str, Any]], experiment_kind: str) -> dict[str, Any]:
...
def _build_diagnostics(
    ...
    overall_rows = [row for row in _build_metrics(prediction_rows, experiment_kind)["summary"] if row["dataset"] == "overall"]
    recommendation = None
    if experiment_kind == "content_ablation":
    ...
    elif experiment_kind == "auditing_ablation":
...
def _estimate_work(
    ...
    for benchmark in benchmarks:
        ...
        elif experiment.experiment_kind == "auditing_ablation":
