...
from research_experiments.workspace.cache_layout_migration import normalize_cache_layout
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(description="查看缓存分片，或将旧分片归并到新的 canonical 布局。")
    subparsers = parser.add_subparsers(dest="command", required=True)

    summarize_cmd = subparsers.add_parser("summarize", help="汇总缓存分片数量、大小与请求条数。")
    summarize_cmd.add_argument("--cache-root", default=default_cache_root())
    summarize_cmd.add_argument("--top-shards", type=int, default=10)
    summarize_cmd.add_argument("--json", action="store_true", help="输出 JSON。")

    inspect_cmd = subparsers.add_parser("inspect-target", help="查看某个 provider/model/dataset 对应的缓存分片。")
    inspect_cmd.add_argument("--cache-root", default=default_cache_root())
    inspect_cmd.add_argument("--provider", required=True)
    inspect_cmd.add_argument("--request-model", required=True)
    ...
    normalize_cmd = subparsers.add_parser(
        "normalize-layout",
        help="把旧 dataset namespace 分片归并到新的 canonical dataset slug 分片。",
    )
    normalize_cmd.add_argument("--cache-root", default=default_cache_root())
    normalize_cmd.add_argument("--apply", action="store_true", help="实际执行归并并删除旧分片。")
    normalize_cmd.add_argument("--json", action="store_true", help="输出 JSON。")
    return parser
...
def main(argv: list[str] | None = None) -> None:
    ...
    if args.command == "normalize-layout":
        payload = normalize_cache_layout(args.cache_root, apply=bool(args.apply))
        if args.json:
            emit_json(payload)
            return
        _print_layout_normalization(payload)
        return
...
def _print_root_summary(summary: CacheRootSummary, top_shards: int) -> None:
    ...
    print(f"分片数量: {summary.shard_count}")
    ...
    print(f"最大分片 Top {min(top_shards, len(summary.shards))}:")
...
def _print_layout_normalization(payload: dict[str, object]) -> None:
    """输出缓存布局归并计划或执行结果。"""
    ...
    for row in candidates:
        if not isinstance(row, dict):
            continue
        target_count = row.get("target_request_count")
        if target_count is None:
            target_count = row.get("target_request_count_before")
        print(
            "- "
            f"{row.get('source_dataset')} -> {row.get('target_dataset')} | "
            f"source={row.get('source_request_count')} | "
