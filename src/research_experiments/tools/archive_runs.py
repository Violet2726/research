...
from research_experiments.core.foundation.run_archives import fetch_run_from_hub, publish_run_to_hub
from research_experiments.core.foundation.workspace import default_runs_hf_repo, workspace_layout


def build_parser() -> argparse.ArgumentParser:
    """构造 run 归档命令行参数。"""

    parser = argparse.ArgumentParser(description="发布或回拉单个科研 run 归档。")
    subparsers = parser.add_subparsers(dest="command", required=True)

    publish_run = subparsers.add_parser("publish-run", help="把单个本地 run 发布到 Hugging Face dataset repo。")
    ...
    fetch_run = subparsers.add_parser("fetch-run", help="从 Hugging Face dataset repo 回拉单个 run。")
