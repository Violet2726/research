...
def _render_markdown(
    ...
    lines.extend(
        [
            ...
            f"- Control Prompt：`{manifest.get('control_prompt_version', 'single_agent_free_text_v1')}`",
            f"- Control Output Protocol：`{manifest.get('control_output_protocol', 'free_text_answer_v1')}`",
            f"- Stage A Response Format：`{manifest.get('stage_a_response_format_mode', 'json_object')}`",
            f"- Adaptive Response Format：`{manifest.get('adaptive_response_format_mode', 'json_object')}`",
            f"- Legacy JSON Mode：`{str(bool(manifest.get('legacy_json_mode', False))).lower()}`",
            f"- 运行目录：`{run_dir.as_posix()}`",
            "",
            "## 研究问题与实验设计",
            "",
            "- 当前快速默认实验只比较 `hetero_vote_3` 与 `cot_1 / sc_3 / sc_5`，先确认异质 Stage A 是否能稳定打败强 no-comm 基线。",
            "- `cot_1 / sc_3 / sc_5` 的 control 路径与 baseline 共用 `single_agent_free_text_v1 + free_text_answer_v1`，便于直接做同口径对比。",
    ...
    lines.extend(
        [
            "## 结论与建议",
            "",
            "- 如果 `hetero_vote_3` 稳定强于 `cot_1 / sc_3 / sc_5`，说明异质 Stage A 仍然是当前最值得继续加码的主线。",
