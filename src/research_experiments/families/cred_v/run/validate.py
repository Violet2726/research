...
def validate_run(run_dir: str | Path, *, family_name: str = "cred_v") -> dict[str, Any]:
    ...
    diagnostic_paths = named_diagnostic_paths(root, family_name=family_name)
    export_paths = named_export_paths(root, family_name=family_name)
    required_paths = [
        index.manifest_path,
...
def _router_check(manifest: dict[str, Any], router_rows: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    required = bool({"cred_v_task_verify_v3", "cred_verify_safe_v1", "cred_acs_v1", "cred_rfs_adaptive_sc_v1"} & set(cred_methods))
    return {
