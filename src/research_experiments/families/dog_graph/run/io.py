...
class RunPaths:
    ...
    relation_selection_trace: Path
    simplification_trace: Path
    answer_attempt_trace: Path
    final_predictions: Path
    metrics: Path
    cost_breakdown: Path
    graph_diagnostics: Path
    run_summary: Path
    run_validation: Path
    progress: Path
...
def _prepare_run_paths(run_root: str | Path, experiment_name: str, phase_name: str, run_id: str) -> RunPaths:
    ...
    return RunPaths(
        ...
        graph_trace=root / "graph_trace.jsonl",
        retrieval_trace=root / "retrieval_trace.jsonl",
        relation_selection_trace=root / "relation_selection_trace.jsonl",
        simplification_trace=root / "simplification_trace.jsonl",
        answer_attempt_trace=root / "answer_attempt_trace.jsonl",
        final_predictions=root / "final_predictions.jsonl",
        metrics=root / "metrics.json",
        cost_breakdown=root / "cost_breakdown.json",
        graph_diagnostics=root / "graph_diagnostics.json",
