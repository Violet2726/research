...
from research_experiments.families.table_critic.config import load_benchmarks, load_protocol_config, phase_metadata


SPEC = get_family_spec("table_critic")


def build_parser() -> argparse.ArgumentParser:
    return build_standard_family_parser(
        family_name=SPEC.family_name,
        description="Table-Critic reproduction runner.",
        inspect_help="Show the resolved Table-Critic experiment configuration.",
        run_help="Execute one configured Table-Critic experiment phase.",
        summarize_help="Print a concise Table-Critic run summary.",
        validate_help="Run Table-Critic validation checks.",
        report_help="Regenerate the Chinese Table-Critic markdown report.",
    )
