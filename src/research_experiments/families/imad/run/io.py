...
class RunPaths:
    ...
    round_diagnostics: Path
    final_predictions: Path
    metrics: Path
    cost_breakdown: Path
    stability_diagnostics: Path
    run_summary: Path
    run_validation: Path
    progress: Path
...
def _prepare_run_paths(run_root: str | Path, experiment_name: str, phase_name: str, run_id: str) -> RunPaths:
    ...
    return RunPaths(
        ...
        round_diagnostics=root / "round_diagnostics.jsonl",
        final_predictions=root / "final_predictions.jsonl",
        metrics=root / "metrics.json",
        cost_breakdown=root / "cost_breakdown.json",
        stability_diagnostics=root / "stability_diagnostics.json",
