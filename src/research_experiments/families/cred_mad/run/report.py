...
def load_metrics(run_dir: str | Path) -> dict[str, Any]:
    return load_metrics_payload(run_dir, family_name="cred_mad")
...
def render_report(run_dir: str | Path, publish_dir: str | Path | None = None) -> dict[str, Any]:
    publish_dir = publish_dir or default_reports_root("cred_mad")
...
def _render_markdown(
    ...
    abstract = [
        "CRED-MAD 将多智能体辩论拆成预提交契约、定向反驳、防守和生存分数聚合，目标是验证辩论是否带来正向纠错漂移。",
    ]
    if best is not None:
