...
def test_retired_prompt_versions_are_not_supported() -> None:
    sample = _sample("gsm8k")
    for retired_prompt_version in ("unified_control_v1_port", "zero_shot_cot_v1"):
        with pytest.raises(ValueError, match="Unsupported single-agent prompt_version"):
            build_messages(sample, method_family="cot", prompt_version=retired_prompt_version)
