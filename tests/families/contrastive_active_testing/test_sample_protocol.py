...
def _d4_output_protocol_fixture() -> DatasetSample:
    return DatasetSample(
        dataset="bbeh",
        sample_id="d4-output-protocol",
        question="Choose yes.\nOptions:\n(A) yes\n(B) no",
        reference_answer="A",
        prompt_context="",
        metadata={
            ...
            "answer_contract": {"kind": "multiple_choice"},
        },
    )


def _d4_output_experiment(d4_output: dict[str, object]) -> SimpleNamespace:
    return SimpleNamespace(
        global_seed=42,
        raw={
            "d4_output": d4_output,
            "phases": {"development": {"evaluation_role": "d4_output_protocol_ab_unit"}},
        },
    )


def test_d4_output_prompt_variants_preserve_legacy_cache_and_high_token_cap(monkeypatch) -> None:
    sample = _d4_output_protocol_fixture()
    protocol = SimpleNamespace(
        stage_candidates=5,
        solver_max_tokens=16_384,
        protocol_version="catch_kernel_v1",
    )
    calls: list[dict[str, object]] = []

    def answer_turn(_sample, **kwargs):
        calls.append(kwargs)
        return {
            ...
            "normalized_answer": "A",
            "prediction": "A",
            "validated_output": {"reasoning": "yes is requested", "final_answer": "A"},
    ...
    sample_runner.run_catch_kernel_d4_sample(
        **common,
        experiment=_d4_output_experiment({"stage_a_protocol": "reasoning_first_json"}),
    )
    assert {call["max_tokens"] for call in calls} == {16_384}
    assert {call["prompt_version"] for call in calls} == {"single_agent_consistent_json_v2"}
    legacy_messages = sample_runner.build_cot_messages(
        sample,
        1,
        prompt_version="single_agent_consistent_json_v2",
    )
    assert "under 80 words" not in legacy_messages[-1]["content"]

    calls.clear()
    sample_runner.run_catch_kernel_d4_sample(
        **common,
        experiment=_d4_output_experiment(
            {
                "stage_a_protocol": "answer_first_json",
                "prompt_variant": "short_reasoning_v3",
            }
        ),
    )
    assert {call["max_tokens"] for call in calls} == {16_384}
    assert {call["prompt_version"] for call in calls} == {"single_agent_answer_first_json_v2"}
    short_messages = sample_runner.build_cot_messages(
        sample,
        1,
        prompt_version="single_agent_answer_first_json_v2",
    )
    assert all("80 words" in message["content"] for message in short_messages)


def test_d4_output_prompt_variant_rejects_unknown_values() -> None:
    with pytest.raises(ValueError, match="Unsupported D4 Stage-A prompt variant"):
        sample_runner.run_catch_kernel_d4_sample(
            _d4_output_protocol_fixture(),
            run_id="run",
            split_name="dev",
            experiment=_d4_output_experiment(
                {"stage_a_protocol": "answer_first_json", "prompt_variant": "typo"}
            ),
            protocol=SimpleNamespace(
                stage_candidates=5,
                solver_max_tokens=16_384,
