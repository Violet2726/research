...
def test_hsgsa_is_historical_only_and_keeps_canonical_replay_metadata() -> None:
    experiment = load_experiment_config(EXPERIMENT)
    registry = load_version_registry(experiment.version_registry)
    protocol = load_protocol_config(experiment.protocol)
    assert registry.active_version is experiment.active_version is None
    assert [key for key, value in registry.versions.items() if value.status == "active"] == []
    assert registry.versions["v5_hsgsa"].status == "retired_superseded"
...
def test_historical_versions_cannot_run() -> None:
    experiment = load_experiment_config(EXPERIMENT)
    registry = load_version_registry(experiment.version_registry)
    ...
    with pytest.raises(ValueError, match="historical-only"):
        require_active_version(registry, None)
    with pytest.raises(ValueError, match="cannot be run"):
        require_active_version(registry, "v5_hsgsa")
