...
def test_build_run_matrix_counts_expected() -> None:
    ...
    assert len(matrix.semantic_entries) == 18
    assert semantic_counts["pending"] == 18
    assert entry_counts["excluded"] == 0
    assert matrix.counts["semantic_unique_targets"] == 18
    ...
    assert imad_entry.primary_method_name == "imad_adaptive"
    assert imad_entry.full_comm_reference == "mad_fixed_r3"
    econ_entry = next(entry for entry in matrix.semantic_entries if entry.experiment_name == "econ_same_context_main")
    assert econ_entry.evaluation_track == "same_context"
    assert econ_entry.evidence_tier == "supporting"
    assert econ_entry.primary_method_name == "econ_bne_main"
