...
def test_canonical_count_splits_are_nested_seed42_prefixes(tmp_path: Path) -> None:
    source_path = tmp_path / "gsm8k.jsonl"
    source_path.write_text(
    ...
    generate_split_manifests([benchmark], tmp_path / "splits")
    dataset_key = benchmark.cache_namespace or benchmark.slug
    count20 = load_split_ids(dataset_key, "count20_seed42", tmp_path / "splits")
    count100 = load_split_ids(dataset_key, "count100_seed42", tmp_path / "splits")
    count300 = load_split_ids(dataset_key, "count300_seed42", tmp_path / "splits")

    assert count20 == count100[:20]
    assert count100 == count300[:100]


def test_noncanonical_split_name_is_rejected(tmp_path: Path) -> None:
    source_path = tmp_path / "gsm8k.jsonl"
    source_path.write_text(
        "\n".join(
            json.dumps({"question": f"Compute {index}+1.", "answer": f"#### {index + 1}"})
    ...
    with pytest.raises(ValueError, match="non-canonical split"):
        generate_split_manifests([benchmark], tmp_path / "splits")
